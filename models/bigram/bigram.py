...
class Bigram:
    ...
    def train_weights(self, train_data: str, epochs: int, learning_rate: float, regularization: float,  generator: torch.Generator = None):
        ...
        xenc = F.one_hot(xs, num_classes=len(self.vocab)).float() # NNs want float inputs!
        # for each batch of inputs shape=(X, len(self.vocab)) where X is the batch_size and each input (row) is a char one hot encoded
        # we want an output shape=(X, len(self.vocab)) -> for each input a probability distribution (a prob for each possible char)
        ...
        # the output distribution for each input is the activation value of the 27 neurons
        # our NN is 1 linear layer of 27 neurons
    ...
    def infer_weights(self, context: str, generator: torch.Generator = None):

        out = []
        i = self.stoi[context[-1]] # because it is a bigram model, we only care about the last character in the context
        while True:
            xenc = F.one_hot(torch.tensor([i]), num_classes=len(self.vocab)).float()
            logits = xenc @ self.W # predict log-counts
            counts = logits.exp() # counts, equivalent to N
    ...
    def loss_counts(self, data: str):
        ...
        # log_likelihood: we can do log(likelihood) to get more manageble numbers (bigger numbers)
        ...
        # recall: let a,b,c be probs: log(a*b*c) = log(a) + log(b) + log(c)
...
def main_counts():
    ...
    print('Loss value of all training set:', model.loss_counts(train_data).item())
    print('Input:', input)
    ...
    for _ in range(10):
        ...
        print('Loss value for above output:', model.loss_counts(output_data).item())
        print('----')
...
def main_weights():
    ...
    train_data = open('data/input.txt', 'r').read()
    g = torch.Generator().manual_seed(123)
    model.train_weights(train_data=train_data, epochs=10, learning_rate=10, regularization=0.01, generator=g)
    ...
    print('----')
    for _ in range(10):
        output_data = model.infer_weights(input, g)
        print('Output:', output_data)
        # print('Loss value for above output:', model.loss_counts(output_data).item())
        print('----')
...
# TODO: try to combine loss computation from the two approaches into one function
